...
private logField(evt, Closure c) {
    ...
    httpPostJson(uri: "https://thinglayer.firebaseio.com/events/${evt.deviceId}/${evt.name}.json",   body:[device: evt.deviceId, name: evt.name, value: evt.value, date: evt.isoDate, unit: evt.unit]) {
        log.debug evt.name+" Event data successfully posted from "+ evt.source
    }
}
